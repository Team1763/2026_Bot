...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(m_driverController, 2) // 2 should be top of joystick
        .whileTrue(new RunCommand(() -> m_intake.runIntakeCommand(), m_robotDrive));

    // Reverse intake
    new JoystickButton(m_driverController, 3) // 3 should be top left of joystick
        .whileTrue(new RunCommand(() -> m_intake.runExtakeCommand(), m_robotDrive));

    // Run feeder
    new JoystickButton(m_driverController, 4) // 4 should be top right of joystick
        .whileTrue(new RunCommand(() -> m_shooter.runFeederCommand(), m_robotDrive));

    // Run feeder
    new JoystickButton(m_driverController, 5) // 4 should be top right of joystick
        .whileTrue(new RunCommand(() -> m_shooter.runFlywheelCommand(), m_robotDrive));
